...
public class TomcatLogParser implements HttpLogParser {
	protected static final Logger log = Logger.getLogger(TomcatLogParser.class);

	/*
	 * TODO should these fields be public? They don't appear to be used
	 * externally.
	 * 
	 * Also, are they any different from HTTPConstants.GET etc. ? In some cases
	 * they seem to be used as the method name from the Tomcat log. However the
	 * RMETHOD field is used as the value for HTTPSamplerBase.METHOD, for which
	 * HTTPConstants is most approriate.
	 */
	 ...
	 */
	public TomcatLogParser(String source) {
		setSourceFile(source);
	}

	/**
	 * by default decode is set to true. if the parameters shouldn't be decoded,
	 ...
	 * 
	 * @return <code>true</code> if parameter values should be decoded,
	 ...
	 *
	 * @param filter
	 *            {@link Filter} to be used while reading the log lines
	 */
	 ...
	 *
	 * @param el
	 *            TestElement to read the lines into
	 * @param parseCount
	 *            number of max lines to read
	 * @return number of read lines, or <code>-1</code> if an error occurred
	 *         while reading
	 */
	public int parse(LogParserCallback callback, int parseCount) {
		if (this.SOURCE == null) {
		...
		try {
			...
			return parse(this.READER, callback, parseCount);
	...
	public int parseAndConfigure(int count, LogParserCallback callback) {
		return this.parse(callback, count);
	}
	 ...
	 *
	 * @param breader
	 *            {@link BufferedReader} to read lines from
	 * @param callback
	 *            {@link TestElement} to read lines into
	 * @param parseCount
	 *            number of lines to read
	 ...
	 */
	protected int parse(BufferedReader breader, LogParserCallback callback,
			int parseCount) {
		int actualCount = 0;
		String line = null;
		try {
			...
			while (line != null) {
				if (line.length() > 0) {
					actualCount += this.parseLine(line, callback);
				}
	 ...
	 *
	 * @param line
	 *            single line to be parsed
	 * @param callback
	 *            {@link TestElement} in which the line will be added
	 * @return number of lines parsed (zero or one, actually)
	 */
	protected int parseLine(String line, LogParserCallback callback) {
		int count = 0;
		...
		callback.setProperty(HttpConstant.METHOD, RMETHOD);
		if (FILTER != null) {
			log.debug("filter is not null");
			if (!FILTER.isFiltered(line, callback)) {
			...
			// parse all the lines
			createUrl(cleanedLine, callback);
		}
		return count;
	}
	...
	/**
	 * @param line
	 *            single line of which the url should be extracted
	 * @param callback
	 *            {@link TestElement} into which the url will be added
	 */
	private void createUrl(String line, LogParserCallback callback) {
		String paramString = null;
		// check the URL for "?" symbol
		paramString = this.stripFile(line, callback);
		if (paramString != null) {
			this.checkParamFormat(line);
			// now that we have stripped the file, we can parse the parameters
			this.convertStringToJMRequest(paramString, callback);
		}
	}
	 ...
	 *
	 * @param entry
	 *            line from which the url is to be extracted
	 * @return cleaned url
	 */
	...
	/**
	 * The method checks for <code>POST</code>, <code>GET</code> and
	 * <code>HEAD</code> methods currently. The other methods aren't supported
	 * yet.
	 *
	 * @param text
	 *            text to be checked for HTTP method
	 * @return <code>true</code> if method is supported, <code>false</code>
	 *         otherwise
	 */
	 ...
	 *
	 * @param url
	 *            url which should be stripped from parameters
	 * @param callback
	 *            {@link TestElement} to parse url into
	 * @return String presenting the parameters, or <code>null</code> when none
	 *         where found
	 */
	public String stripFile(String url, LogParserCallback callback) {
		if (url.indexOf('?') > -1) {
			StringTokenizer tokens = this.tokenize(url, "?");
			this.URL_PATH = tokens.nextToken();
			callback.setProperty(HttpConstant.PATH, URL_PATH);
			return tokens.hasMoreTokens() ? tokens.nextToken() : null;
		}
		callback.setProperty(HttpConstant.PATH, url);
		return null;
	}

	/**
	 * Checks the string to make sure it has <code>/path/file?name=value</code>
	 * format. If the string doesn't contains a "?", it will return
	 * <code>false</code>.
	 *
	 * @param url
	 *            url to check for parameters
	...
	/**
	 * Checks the string to see if it contains "&amp;" and "=". If it does,
	 * return <code>true</code>, so that it can be parsed.
	 *
	 * @param text
	 *            text to be checked for <code>&amp;</code> and <code>=</code>
	 * @return <code>true</code> if <code>text</code> contains both
	 *         <code>&amp;</code> and <code>=</code>, <code>false</code>
	 *         otherwise
	 */
	 ...
	 *            to be be converted
	 * @param callback
	 *            {@link HTTPSamplerBase} which consumes the <code>text</code>
	 */
	public void convertStringToJMRequest(String text, LogParserCallback callback) {
		callback.parseQueryAndArguments(text);
	}
	 ...
	 *
	 * @param stringparams
	 *            String with parameters to be parsed
	 ...
	 * pairs are separated by the ampersand symbol "&amp;". Someone could write
	 * the querystrings by hand, but that would be round about and go against
	 ...
	 * @param delim
	 *            delimiter
	 * @return StringTokenizer constructed with <code>line</code> and
	 *         <code>delim</code>
	 */
